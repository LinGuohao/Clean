...
soRich x = hd [((m.owner).name) \\m<-: x|(toInt(m.balance)) == last(sort[toInt(z.balance) \\ z <-: x | not((z.owner).isDead)])]




//Start = f2 {A01,A02,A03}
//soRich :: {Account} -> String
...
query (f,x,y) q =  {z \\z <-: q | (f z) && ((z.dateCreated).year)> x && ((z.dateCreated).year) < y }
