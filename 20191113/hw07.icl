...
/*
Write a function that takes an Int and an
array of Accounts updates each Account
in an array by adding the number to the balance.
*/
 
gimmeMoney :: {Account} Real-> {Account}
gimmeMoney a b = { {number = x.number, owner = x.owner, balance = x.balance + b , dateCreated = x.dateCreated}\\ x <-: a  } 
...
soRich n = hd [x.owner.name \\ x <-: n | x.balance == Money n]


Money :: {Account} -> Real
Money n =  last (st (F {x \\ x <-: n | x.owner.isDead == False}))
...
st :: [Real] -> [Real]
st [] = []
st [a:b] = st [x \\ x <- b | x < a] ++ [a] ++ st [x \\ x <- b | x >= a]
...
and returns an list containing all accounts
...
query (n,x,y) z = {m \\ m <-: z | (n m) && m.dateCreated.year > x && m.dateCreated.year < y }
